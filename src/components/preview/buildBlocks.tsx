...
/** Frame styles for header contact icons (0 = plain, then filled/tinted/outlined variants). */
function iconFrame(style: number | undefined, accent: string, onAccent: boolean): CSSProperties {
  const c = onAccent ? '#fff' : accent;
  switch (style) {
    case 1: return { background: c, color: '#fff', borderRadius: '50%', padding: 3 };
    case 2: return { background: c, color: '#fff', borderRadius: 4, padding: 3 };
    case 3: return { background: c, color: '#fff', padding: 3 };
    case 4: return { border: `1px solid ${c}`, borderRadius: '50%', padding: 3 };
    case 5: return { border: `1px solid ${c}`, borderRadius: 4, padding: 3 };
    case 6: return { border: `1px solid ${c}`, padding: 3 };
    default: return {};
  }
}
...
function ContactLine({ p, ctx, icons }: { p: PersonalInfo; ctx: RenderContext; icons: boolean }): ReactNode {
  ...
  const delimiter = ctx.headerDelimiter ?? 'icon';
  const framed = (ctx.headerIconStyle ?? 0) > 0;
  const iconAccent = ctx.accentTargets?.headerIcons && !ctx.onAccent && !framed ? ctx.accent : undefined;
  const frameStyle = iconFrame(ctx.headerIconStyle, ctx.accent, ctx.onAccent);
  const ico = (I: typeof Mail) =>
    icons && delimiter === 'icon' ? <I size={11} style={{ flexShrink: 0, color: iconAccent, ...frameStyle }} /> : null;
  const items: { icon: ReactNode; text: string; href?: string; scope?: keyof NonNullable<RenderContext['linkScope']> }[] = [];
  if (p.email) items.push({ icon: ico(Mail), text: p.email, href: `mailto:${p.email}`, scope: 'email' });
  if (p.phone) items.push({ icon: ico(Phone), text: p.phone, href: `tel:${p.phone.replace(/[^\d+]/g, '')}`, scope: 'phone' });
  ...
  if (p.linkedin) items.push({ icon: ico(Linkedin), text: p.linkedin, href: withHttp(p.linkedin), scope: 'linkedin' });
  ...
  for (const d of p.details ?? []) {
    ...
    items.push({ icon: ico(Globe), text: d.value || d.label, href: d.href ? withHttp(d.href) : undefined });
  }

  const justify = ctx.headerAlign === 'top' ? 'center' : ctx.headerAlign === 'right' ? 'flex-end' : 'flex-start';
  const sep = delimiter === 'bullet' ? '•' : delimiter === 'bar' ? '|' : '';
  const grid = ctx.headerArrangement === 'grid';
  return (
    <div
      style={{
        display: grid ? 'grid' : 'flex',
        gridTemplateColumns: grid ? 'repeat(2, max-content)' : undefined,
        flexWrap: grid ? undefined : 'wrap',
        gap: '0.35em 0.9em',
        fontSize: '0.85em',
        color,
        marginTop: '0.4em',
        justifyContent: grid ? justify : justify,
      }}
    >
      {items.map((it, i) => {
        // Per-field link scope: when set, only listed fields get link styling.
        const scoped = ctx.linkScope && it.scope ? !!ctx.linkScope[it.scope] : true;
        return (
          <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3em' }}>
            {sep && i > 0 && !grid && <span style={{ opacity: 0.5 }}>{sep}</span>}
            {it.icon}
            {it.href ? (
              <a
                href={it.href}
                ...
                style={{
                  color: scoped && ctx.linkBlue && !ctx.onAccent ? '#2563eb' : 'inherit',
                  textDecoration: scoped && ctx.linkUnderline ? 'underline' : 'none',
                }}
              >
                {it.text}
              </a>
...
function Header({
  ...
  if (banner) {
    const img = ctx.headerImage;
    return (
      <div
        style={{
          background: img ? `linear-gradient(rgba(0,0,0,0.35),rgba(0,0,0,0.35)), url(${img}) center/cover` : accent,
          color: '#fff',
          padding: '1.1em 1.2em',
          borderRadius: 6,
          marginBottom: '0.2em',
...
interface BuildArgs {
   ...
   *  Expanded styles are handled by SectionHeading via ctx.headingOverride;
   *  here we only map the few that have a base TitleStyle equivalent. */
  headingStyle?: import('../../types/resume').HeadingStyleOverride;
  ...
  const header: Block = {
    ...
    node: (
      <Header
        ...
        ctx={headerCtx}
        banner={layout === 'banner' || args.colorArea === 'header' || !!ctxBase.headerImage}
        accent={accent}
